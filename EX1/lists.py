...
def front_x(words):
    lst1 = [word for word in words if word[0] == "x"]
    lst2 = [ word for word in words if word not in lst1 ]
    lst1.sort()
    lst2.sort()
    result = lst1 + lst2
    return result
...
def linear_merge(sorted1, sorted2):
    ...
    ptr2 = 0
    lst = []
    while ptr1 != len(sorted1)  or ptr2 != len(sorted2):
        if ptr1 == len(sorted1) :
            lst += sorted2[ptr2:]
            break
        elif ptr2 == len(sorted2) :
            lst += sorted1[ptr1:]
            break
        elif sorted1[ptr1] <= sorted2[ptr2]:
            lst.append(sorted1[ptr1])
            ptr1 += 1
        else:
            lst.append(sorted2[ptr2])
            ptr2 += 1
    return lst
